...
function buildCharacter() {
  ...
  let descriptor = descriptors[character.descriptor]; //TODO: Add points from leveling up
  ...
  $('input[id$=-bonus]').each(function () {
    ...
    let maxValue = parseInt($(this).attr('max')); //No malarky here
  ...
  $('#remaining-bonus').html(bonusPoints - bonusSpent); //Calculate derived stats

  let pool = Object.assign({}, types[character.type].pool); //TODO: Handle for special luck pool from Lucky descriptor
  ...
  localStorage.character = JSON.stringify(character); //TODO: Add skills from other sources

  $('#skills').empty();
  let skillList = []; //Pretty sure all descriptors have skills, but just in case
  ...
  let traitList = []; //They definitely don't all have traits.
...
function toSentenceCase(targetString) {
  return targetString.replace(/(^\s*\w|[\.\!\?]\s*\w)/g, function (c) {
